...
const FeatureCard = memo<FeatureCardProps>(({ feature }) => {
  return (
    <Card 
      className="bg-zinc-900/50 border-zinc-800/50 backdrop-blur-sm rounded-2xl hover:bg-zinc-800/50 transition-all duration-200 group relative overflow-hidden" 
      style={{ 
    ...
    >
      <div 
        className={`absolute inset-0 bg-gradient-to-br ${feature.gradient} opacity-0 group-hover:opacity-5 transition-opacity duration-200`}
        style={{ 
          willChange: 'opacity',
          backfaceVisibility: 'hidden'
        }}
      ...
      <CardContent className="p-6 sm:p-8 relative z-10">
        <div 
          className={`w-12 h-12 sm:w-16 sm:h-16 bg-gradient-to-br ${feature.gradient} rounded-2xl flex items-center justify-center mb-4 sm:mb-6 group-hover:scale-110 transition-transform duration-200 shadow-lg`} 
          style={{ 
            transform: 'translate3d(0,0,0)',
        ...
        <h3 className="text-lg sm:text-xl font-bold text-white mb-3 sm:mb-4 group-hover:text-zinc-100 transition-colors duration-200">
          {feature.title}
        </h3>
        <p className="text-zinc-400 leading-relaxed group-hover:text-zinc-300 transition-colors duration-200 text-sm sm:text-base">
          {feature.description}
        </p>
        <div 
          className={`absolute bottom-0 left-0 w-full h-1 bg-gradient-to-r ${feature.gradient} transform scale-x-0 group-hover:scale-x-100 transition-transform duration-200 origin-left`} 
          style={{ 
